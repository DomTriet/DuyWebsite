  ...
  template: `
    <div class="min-h-screen bg-gradient-to-b from-white via-gray-50 to-gray-100">
      <div class="max-w-5xl mx-auto px-6 py-16">
        <!-- Back Button -->
        <a routerLink="/" class="inline-flex items-center text-gray-500 hover:text-gray-900 font-semibold mb-12 transition-colors group">
        ...
        <div class="flex justify-between items-start md:items-center gap-6 mb-12 flex-col md:flex-row">
          ...
          <a routerLink="/forum/create" style="background:#0D0D0D; color:#F7F6F3; padding:12px 22px; border-radius:8px; font-weight:700; font-size:0.875rem; text-decoration:none; white-space:nowrap; transition:background 0.2s;" onmouseover="this.style.background='#1a1a1a'" onmouseout="this.style.background='#0D0D0D'">{{ 'FORUM_PAGE.NEW_POST' | translate }}</a>
        </div>
          ...
          <div *ngFor="let post of posts" class="bg-white rounded-2xl shadow-md border border-gray-100 hover:shadow-lg transition-all duration-300 overflow-hidden hover:-translate-y-0.5">
            ...
            <div class="px-6 py-4 bg-gray-50 flex items-center justify-between">
              <div class="flex items-center gap-6">
                <span class="flex items-center gap-2 text-gray-600 hover:text-gray-900 cursor-pointer transition-colors text-sm font-medium">
                  <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M4.318 6.318a4.5 4.5 0 000 6.364L12 20.364l7.682-7.682a4.5 4.5 0 00-6.364-6.364L12 7.636l-1.318-1.318a4.5 4.5 0 00-6.364 0z"></path></svg>
                  {{ 'FORUM_PAGE.LIKE' | translate }}
                </span>
                <a [routerLink]="['/forum', post.id]" class="flex items-center gap-2 text-gray-600 hover:text-gray-900 transition-colors text-sm font-medium">
                  <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M8 12h.01M12 12h.01M16 12h.01M21 12c0 4.418-4.03 8-9 8a9.863 9.863 0 01-4.255-.949L3 20l1.395-3.72C3.512 15.042 3 13.574 3 12c0-4.418 4.03-8 9-8s9 3.582 9 8z"></path></svg>
                  {{ 'FORUM_PAGE.COMMENT' | translate }}
                </a>
              </div>
              <a [routerLink]="['/forum', post.id]" style="color:#374151; font-weight:700; font-size:0.8rem; text-decoration:none;" onmouseover="this.style.color='#0D0D0D'" onmouseout="this.style.color='#374151'">{{ 'FORUM_PAGE.VIEW_DETAIL' | translate }} →</a>
            </div>
          </div>
