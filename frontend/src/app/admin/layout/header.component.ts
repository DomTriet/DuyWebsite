  ...
  template: `
    <header class="bg-white border-b border-gray-200 z-10 py-4 px-6 flex items-center justify-between shadow-sm">
      ...
      <div class="flex items-center gap-4">
        <!-- Notification -->
        <div class="relative">
          <button (click)="toggleNotifications()" class="text-gray-600 hover:text-indigo-600 transition-colors relative p-2 rounded-lg hover:bg-gray-100 focus:outline-none">
            <svg class="h-6 w-6" fill="none" viewBox="0 0 24 24" stroke="currentColor">
          ...
          <div *ngIf="showNotifications" @dropdownAnimation class="absolute right-0 mt-2 w-80 bg-white rounded-xl shadow-xl border border-gray-200 z-50 overflow-hidden">
            <div class="p-4 flex justify-between items-center border-b border-gray-100 bg-gray-50">
              ...
              <button (click)="markAllAsRead()" class="text-xs text-indigo-600 hover:text-indigo-800 font-medium focus:outline-none">Đánh dấu đã đọc</button>
            </div>
            <div class="max-h-96 overflow-y-auto">
              ...
              <a *ngFor="let notification of notifications" [routerLink]="notification.link" (click)="markAsRead(notification.id)"
                 class="flex items-start gap-4 p-4 hover:bg-indigo-50/50 border-b border-gray-50 transition-colors cursor-pointer"
                 [ngClass]="{'bg-white': notification.read, 'bg-indigo-50/30': !notification.read}">
                <div class="w-2 h-2 rounded-full mt-2 flex-shrink-0" [ngClass]="notification.read ? 'bg-gray-300' : 'bg-indigo-500'"></div>
                <div class="flex-1">
